...
def sample(G, z_, y_, config):
  with torch.no_grad():
    ...
    y_.sample_()
    if config['parallel']:
      G_z =  nn.parallel.data_parallel(G, (z_, G.shared(y_)), device_ids=[0, 1, 2, 4, 5])
    else:
...
def sample_sheet(G, classes_per_sheet, num_classes, samples_per_class, parallel,
  ...
  for i in range(num_classes // classes_per_sheet):
    ...
    for j in range(samples_per_class):
      ...
      with torch.no_grad():
        if parallel:
          o = nn.parallel.data_parallel(G, (z_[:classes_per_sheet], G.shared(y)), device_ids=[0, 1, 2, 4, 5])
        else:
...
def interp_sheet(G, num_per_sheet, num_midpoints, num_classes, parallel,
  ...
  with torch.no_grad():
    if parallel:
      out_ims = nn.parallel.data_parallel(G, (zs, ys), device_ids=[0, 1, 2, 4, 5]).data.cpu()
    else:
